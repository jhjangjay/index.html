...
<title>화성 지식 & 일상 대화 – Free AI (WebLLM 3B)</title>
<style>
...
<body>
<div id="wrap">
  <h1>화성에 대해 무엇이든 물어보세요 <small>Ask anything about Mars • 일상 대화도 가능</small></h1>
  <div class="capsule">
    <input id="q" placeholder="예) 화성 대기는 어떻게 구성돼? / 오늘 기분이 안좋아…" />
...
(function(){
  ...
  // ===== 간단 분류 (정보 vs 일상) =====
  const SMALLTALK=/(기분|힘들|우울|행복|고마|고맙|응원|상담|조언|대화|심심|농담|웃긴)/i;

  // ===== 대화 히스토리 (브라우저 저장) =====
  const KEY='mars_chat_history_v1';
  let history=JSON.parse(localStorage.getItem(KEY)||'[]'); // [{role, content}]
  function save(){ localStorage.setItem(KEY, JSON.stringify(history.slice(-12))); }
  ...
  async function nasaAPOD(){
    try{const r=await fetch('https://api.nasa.gov/planetary/apod?api_key=DEMO_KEY');if(!r.ok)return null;return await r.json()}catch(_){return null}
  }
  ...
  async function geocode(place){
    const r=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(place)}&count=1&language=ko&format=json`);
    if(!r.ok) return null; const j=await r.json(); return j.results?.[0]||null;
  }
  ...
  // ===== WebLLM (3B 우선, 실패시 1B) =====
  let engine=null, ready=false, loading=false, tried=false;
  async function ensureLLM(){
    if(ready||loading||tried) return; loading=true; tried=true;
    if(!('gpu' in navigator)){ note.textContent='⚠️ WebGPU 미지원: 검색 요약 + 경량 답변 모드로 동작합니다. (Chrome/Edge 최신 권장)'; loading=false; return; }
    for(const m of ['Llama-3.2-3B-Instruct-q4f16_1-MLC','Llama-3.2-1B-Instruct-q4f16_1-MLC']){
      try{
        engine = await webllm.CreateMLCEngine(m,{
          initProgressCallback:(p)=>{ const pct=typeof p.progress==='number'?Math.round(p.progress*100):null; note.textContent = pct!=null?`모델 로딩 중… ${pct}% (${m})`:(p.text||`모델 로딩 중… (${m})`); }
        });
        ready=true; loading=false; note.textContent='모델 준비 완료!'; return;
      }catch(e){ console.warn('모델 로딩 실패', m, e); }
    }
    loading=false; note.textContent='온디바이스 LLM 로딩 실패 – 검색 요약 위주로 답합니다.';
  }

  async function llmReply(question, context=''){
    await ensureLLM();
    const baseSystem = '너는 따뜻하고 실용적인 한국어 조수야. 정확성을 우선하지만, 사람이 느끼는 감정에도 공감해줘.';
    const marsSystem  = '너는 화성/우주/기상 분야 공개 자료를 토대로 사실 기반으로 설명하는 전문가야.';
    const system = context ? `${baseSystem}\n${marsSystem}\n- 출처가 없으면 확언하지 말고 보수적으로 답해.` : baseSystem;

    // 히스토리 + 컨텍스트 삽입
    const msgs = [{role:'system',content:system}];
    history.slice(-8).forEach(m=>msgs.push(m));
    if(context){ msgs.push({role:'user', content:`다음 참고 요약을 반영해서 답해줘:\n${context}`}); }
    msgs.push({role:'user', content:question});

    if(!engine){ // LLM 불가 시 간단한 공감 답변
      return SMALLTALK.test(question)
        ? '그럴 때가 있죠. 오늘 스스로를 조금은 쉬게 해주는 게 어때요? 가벼운 산책이나 물 한잔도 도움이 됩니다. 필요하면 이야기 더 해줘요.'
        : (context ? '찾은 공개 자료를 위에 정리했어요. 도움이 되었길 바라요.' : '아직 관련 자료가 부족하지만, 조금 더 구체적으로 물어보면 도와줄게요.');
    }

    const out = await engine.chat.completions.create({
      messages: msgs, temperature: 0.35, max_tokens: 360
    });
    return out?.choices?.[0]?.message?.content || '';
  }

  // ===== 도우미 =====
  const take = (t)=> (t||'').split(/[。\.\n]/).filter(Boolean).slice(0,2).join(' ').slice(0,300);
  const section=(h,html)=>`<div class="sub">${h}</div><div>${html}</div>`;
  const bullets = (a)=>`<ul style="margin:6px 0 0 18px">${a.map(x=>`<li>${x}</li>`).join('')}</ul>`;

  // ===== 메인 처리 =====
  async function answer(){
    const q = qEl.value.trim(); if(!q) return;
    card.style.display='block'; title.textContent='AI 답변'; body.textContent='생각 중…'; statusEl.textContent='';
    ...
    if(WEATHER.test(q)){
      ...
      history.push({role:'user',content:q},{role:'assistant',content:`현재 ${g.name}의 기온은 ${cur.temperature_2m}°C, 체감 ${cur.apparent_temperature}°C.`}); save();
      return;
    }

    // 2) 정보 검색 + 요약 (위키·Wikidata·NASA 등)
    try{
      statusEl.textContent='무료 출처 찾는 중…';
      ...
      // 3) 항상 LLM로 답변 생성(일상도 포함) — 컨텍스트 없으면 대화 답변만
      statusEl.textContent = context ? '자료 요약 중…' : (SMALLTALK.test(q) ? '대화 중…' : '생각 중…');
      const llm = await llmReply(q, context);

      const secs=[];
      if(llm) secs.push(section('요약 답변', llm));
      if(wikis.length){
        secs.push(section('백과사전 요약', bullets(wikis.slice(0,4).map(s=>`${s.title}: ${take(s.extract)}  —  ${s.url?`<a class="muted" href="${s.url}" target="_blank">원문</a>`:''}`))));
      }
      if(wd.length){
        secs.push(section('Wikidata', bullets(wd.slice(0,3).map(x=>`${x.label}: ${x.desc} <span class="muted">[${x.id}]</span>`))));
      }
      ...
      if(papers.length){
        secs.push(section('논문(Top)', bullets(papers.map(p=>`${p.title} (${p.year||''}) — <a href="${p.url}" target="_blank">${p.doi||'링크'}</a>`))));
      }
      if(books.length){
        secs.push(section('도서(Top)', bullets(books.map(b=>`${b.title}${b.author? ' — '+b.author:''}${b.year? ' ('+b.year+')':''} — <a href="${b.url}" target="_blank">OpenLibrary</a>`))));
      }

      body.innerHTML = secs.length? secs.join('') : (llm || '조금 더 구체적으로 물어봐줘!');
      statusEl.textContent='';

      history.push({role:'user',content:q},{role:'assistant',content: llm || '자료 요약을 제공했습니다.'}); save();
    }catch(e){
      console.error(e);
